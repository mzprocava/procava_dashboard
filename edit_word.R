...
payments_data <- dbGetQuery(con, "SELECT * FROM fiduciary.full_payment_dataset")%>% filter(processo_numero == "088/UNGP/PMA&GC/032.22/2022")
# payments_data$supplier_name[1]
# text_fields <- dbGetQuery(con, "SELECT * FROM fiduciary.full_payment_dataset")
...
# sample_doc <- body_add_table(sample_doc, payments_summary, pos = "after", style = 'Table Grid')
...
# sample_doc <- body_replace_all_text(sample_doc, old_value = "paid_ammount", new_value = paste0(payments_data$paid_ammount[1]), only_at_cursor = FALSE, fixed = TRUE)
...
# sample_doc <- body_replace_all_text(sample_doc, old_value = "contractdescription_pt", new_value = paste0(payments_data$contract[1]), only_at_cursor = FALSE, fixed = TRUE)
# sample_doc <- body_replace_all_text(sample_doc, old_value = "compo_desc", new_value = paste0(payments_data$componentes_pt[1]), only_at_cursor = FALSE, fixed = TRUE)
# sample_doc <- body_replace_all_text(sample_doc, old_value = "componente_pt", new_value = paste0(payments_data$components_pt[1]), only_at_cursor = FALSE, fixed = TRUE)
...
print(sample_doc, target = "C:\\Users\\Administrator\\Dropbox\\2022_apps\\pmu\\IP_RMarkdown_Final.docx")
